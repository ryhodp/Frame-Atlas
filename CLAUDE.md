...
frame-atlas/
├── backend/
│   ├── app.py              # Endpoints, database, Drive, tagging, crop worker
...
- **Only pure code could move, and this is why.** All 34 `scripts/test_*_locally.py` copy `app.py` — one single file — into a temp directory, string-patch `DB_PATH` inside it, then import that copy with `backend/` on `sys.path`. Move DB-touching code out and the tests keep running while the moved module resolves from the real `backend/` carrying the production `/app/data/library.db` path. Pure modules have no `DB_PATH`, so nothing in the harness changed. **Extracting anything further is blocked on reworking that harness first** (make the DB path an env var) — that's Day 27 part 2, not an optional cleanup
...
**CI (V43/Day 25)**
- `.github/workflows/tests.yml` runs on every push/PR: every `scripts/test_*_locally.py` script (Python 3.11, matching Railway's deploy image) plus the two pure-logic `.mjs` tests (Node) — 36 scripts total. Every script already builds its own throwaway synthetic database (see any `test_*_locally.py`'s `DB_PATH` patch trick), so this needed no fixtures or secrets checked in, contrary to the original plan
- `test_shuffle_locally.py` is skipped with an explained `::warning::` — one assertion is stale from before V35 dropped recency weighting (flagged in the Day 24 session log as its own fix, not a regression), and shipping CI already-red for something unrelated would undermine the whole point of adding it
